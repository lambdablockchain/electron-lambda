# Electron lambda Update Checker
### (`releases.json`)

This directory contains the `releases.json` file that the Electron lambda update checker uses to determine when a new version of Electron lambda is available.

#### Update Checker Overview
As of version 3.3.6 of Electron lambda, there is an update-checking facility built-in to the Qt desktop app. The facility basically functions as follows:

1. When the user selects "Check for updates...", Electron lambda connects to the URL hard-coded in `gui/qt/update_checker.py` (currently: https://raw.github.com/Electron-Cash/Electron-Cash/master/contrib/update_checker/releases.json)
...
It hopefully will decrease the number of users running very old versions of Electron lambda.

#### What This Means For You, The Maintainer
You need to update `releases.json` in this directory whenever a new version of Electron lambda is released, and push it to master.
...
- And empty/omitted variant means "Electron lambda Regular"
- The variant must match the variant in `lib/version.py`.
    

#### How To Update `releases.json`

  1. Release Electron lambda as normal, updating the version in `lib/version.py`.
...
-  Open up Electron lambda and go to that address in the "Addresses" tab and right click on it, selecting **"Sign/Verify Message"**
...
#### Feedback / Questions
Open up an issue in the issue tracker or contact Calin Culianu (/u/NilacTheGrim on reddit), as he is the author of this mechanism in Electron lambda.
