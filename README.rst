...
::
  ...
  Licence: MIT Licence
  Author: Electron lambda Developers
  Language: Python
...
.. image:: https://d322cqt584bo4o.cloudfront.net/electron-cash/localized.svg
    ...
    :alt: Help translate Electron lambda online
...
**Note: If running from source, Python 3.6 or above is required to run Electron lambda.** If your system lacks Python 3.6,
...
Electron lambda is a pure python application forked from Electrum. If you want to use the Qt interface, install the Qt dependencies::

    sudo apt-get install python3-pyqt5 python3-pyqt5.qtsvg

If you downloaded the official package (tar.gz), you can run
Electron lambda from its root directory (called Electron lambda), without installing it on your
system; all the python dependencies are included in the 'packages'
directory. To run Electron lambda from its root directory, just do::

    ./electron-cash

You can also install Electron lambda on your system, by running this command::

    sudo apt-get install python3-setuptools
    python3 setup.py install

This will download and install the Python dependencies used by
Electron lambda, instead of using the 'packages' directory.

If you cloned the git repository, you need to compile extra files
before you can run Electron lambda. Read the next section, "Development
Version".

Hardware Wallet - Ledger Nano S
-------------------------------

Electron lambda natively support Ledger Nano S hardware wallet. If you plan to use
...
3. ``cd`` into the Electron lambda directory. Run ``pyenv local 3.6.9`` which inserts
...
4. Install Electron lambda requirements: ``python3 -m pip install --user -r contrib/requirements/requirements.txt``
