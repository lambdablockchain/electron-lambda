...
::
  ...
  Licence: MIT Licence
  Author: The Electron lambda Developers; Satochip portions by Toporin
  Language: Python (>= 3.6)
  Homepage:
...
Rem: Electron lambda uses Python 3.x. In case of error, check first that you are not trying to run Electron lambda with Python 2.x or with Python 2.x libraries.
...
In PowerShell, run Electron lambda on the testnet (-v allows for verbose output)::
...
==============================
(Electron lambda requires Python 3.6, which should be installed by default on Ubuntu)
(If necessary, install pip: sudo apt-get install python3-pip)

Electron lambda is a pure python application. To use the
...
To run Electron lambda on the testnet use::
...
If you run the test suite after (or before) Electron lambda, you may block the card if the PIN used are not the same!
