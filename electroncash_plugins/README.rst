Electron lambda - Plugins
=======================

The plugin system of Electron lambda is designed to allow the development
of new features without increasing the core code of Electron lambda.

Electron lambda is written in pure python. if you want to add a feature
...
a remote server (not an Electron lambda server), then it should be a
plugin as well. If the feature you want to add introduces new
dependencies in the code, then it should probably be a plugin.

There are two types of plugins supported by Electron lambda.  The first is the
...
Plugins, like Electron lambda, are written in pure Python, in the form of
...
Optional features (internal plugins) are included with Electron lambda, and are
available to all users of Electron lambda to enable and disable as they wish.
...
- We expect plugin developers to maintain their plugin code. However,
  once a plugin is merged in Electron lambda, we will have to maintain it
  too, because changes in the Electron lambda code often require updates in
...
- Plugins should be compatible with Electron lambda's conventions. If your
  plugin does not fit with Electron lambda's architecture, or if we believe
  that it will create too much maintenance work, it will not be
  accepted. In particular, do not duplicate existing Electron lambda code in
  your plugin.

- We may decide to remove a plugin after it has been merged in
  Electron lambda. For this reason, a plugin must be easily removable,
...
clone of the Electron lambda source you are developing within.

Please be sure that you test your plugin with the same recommended version of
Python for the version of Electron lambda you intend to specify in your
plugin's minimum Electron lambda version.  Not doing so, will cause you pain
...
- ``description``: A longer form description of how your plugin upgrades
  Electron lambda.
- ``minimum_ec_version``: This is the earliest version of Electron lambda
...
- ``package_name``: This is the name of the Python package directory at the
  ...
  necessary to formally specify this, to spare Electron lambda confusion in
...
- ``available_for``: This is a list of keywords which map to supported
  Electron lambda plugin interfaces.  Valid values to include are ``qt`` and
  ``cmdline``.
...
In the ``contrib`` directory of the Electron lambda source tree, you can find a script
...
user can then drag into their Electron lambda wallet's plugin manager, to
...
the Electron lambda developers to work out a standard way to do so, so that if
